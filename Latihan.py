...
def add_expense(expenses, new_expense):
    update_expenses = expenses.copy()
    update_expenses.append(new_expense)
    return update_expenses

calculate_total_expenses = lambda expenses: sum(expense['jumlah'] for expense in expenses)

def get_expenses_by_date(expenses, target_date):
    return [expense for expense in expenses if expense.get('tanggal') == target_date]

def generate_expenses_report(expenses):
    for expense in expenses:
        date = expense.get('tanggal', 'tanggal tidak tersedia')
        amount = expense.get('jumlah', 'jumlah tidak tersedia')
        yield f'tanggal: {date}, jumlah: {amount}'
...
def add_expense_interactively(expenses):
    ...
    expenses = add_expense(expenses, new_expense)
